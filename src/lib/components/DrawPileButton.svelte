...
<div class="absolute right-5 bottom-5">
	{#if eventStore.cards.remaining.length > 0}
		<button
			...
			class="flex h-20 w-12 scale-90 cursor-pointer items-center justify-center rounded-md bg-white p-0.5 shadow-md transition-transform duration-100 ease-in-out hover:scale-100 md:h-40 md:w-24 md:p-1"
			onclick={handleDrawPileClick}
		>
			<div class="flex h-full w-full flex-col items-center justify-center rounded-md bg-red-500">
				<svg
					class="h-14 w-8 md:h-28 md:w-16"
					viewBox="0 0 100 100"
				...
				<span class="text-sm md:text-base">{eventStore.cards.remaining.length}</span>
			</div>
		</button>
	{/if}
	{#each eventStore.cards.remaining as stackedCard, stackPosition (stackedCard.id)}
		...
		>
			<div
				...
				class={[
					'flex h-full flex-col rounded-md border border-gray-200 shadow-sm transition-transform duration-200 ease-in-out select-none'
				]}
			>
				<div
